"use client";
import { Mail, Zap, Palette } from "lucide-react";
...
function Headers() {
  const { toggleBackground, background } = useBackground();
  ...
  return (
    <>
      <motion.header
        ...
        className=" fixed md:top-1.5 top-0 w-full z-50 text-white"
      >
        <nav className="sm:w-[98%] w-[92%] py-2  mx-auto flex text-white items-center  justify-between rounded-md p-1 relative">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              data-cursor-active
              className="border w-8 cursor-pointer hover:w-[7.4rem] relative hover:border-purple-500 hover:bg-purple-600 overflow-hidden flex justify-end group items-center gap-2 transition-all duration-300 border-zinc-800 rounded-full p-2 px-2 bg-zinc-900"
              title={`Current background: ${background}`}
            >
